#ifndef APEXWATER_SURFACE_INCLUDED
#define APEXWATER_SURFACE_INCLUDED

inline ApexSurfaceData ApexWaterBuildSurface(ApexVaryings i, sampler2D baseMap, float4 baseMapST, fixed4 baseColor, sampler2D normalMap, float4 normalMapST, half normalScale, sampler2D maskMap, float4 maskMapST, fixed4 emissionColor)
{
    ApexSurfaceData s;
    ...
    s.smoothness = max(s.smoothness, 0.75h); s.alpha *= 0.65h;
    return s;
}

inline half3 ApexWaterFinish(half3 color, ApexSurfaceData s, ApexVaryings i, half spectraAmount)
{
