Shader "Apex/Water/PoolLite"
{
    Properties
    {
        _BaseMap("Surface Texture", 2D) = "white" {}
        _BaseColor("Surface Tint", Color) = (1,1,1,1)
        _ShallowColor("Shallow Color", Color) = (0.12,0.65,0.75,1)
        _DeepColor("Deep Color", Color) = (0.015,0.08,0.18,1)
        _NormalMap("Water Normal", 2D) = "bump" {}
        _NormalScale("Wave Normal Scale", Range(0,2)) = 0.6
        _NormalSpeedA("Wave Speed A", Vector) = (0.025,0.018,0,0)
        _NormalSpeedB("Wave Speed B", Vector) = (-0.018,0.032,0,0)
        _NormalBlend("Second Wave Blend", Range(0,1)) = 0.7
        _MaskMap("Mask: R Depth, B Foam", 2D) = "black" {}
        _Smoothness("Smoothness", Range(0,1)) = 0.9
        _Opacity("Opacity", Range(0,1)) = 0.65
        [HDR] _FresnelColor("Fresnel Color", Color) = (0.25,0.8,1,1)
        _FresnelPower("Fresnel Power", Range(0.5,8)) = 4
        _FresnelStrength("Fresnel Strength", Range(0,1)) = 0.65
        [HDR] _FoamColor("Foam Color", Color) = (0.8,0.95,1,1)
        ...
        [Enum(Off,0,Albedo,1,Normals,2,MOS,3,UV,4,VertexColor,5,Emission,6,FoamMask,7,BakedGI,8)] _DebugMode("Debug View", Int) = 0
    }
